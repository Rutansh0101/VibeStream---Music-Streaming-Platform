  ...
  return (
    <>
        ...
        <div className='mt-10 flex gap-8 flex-col md:flex-row md:items-end'>
            <img src={albumData.image} alt='image' className='rounded w-48'/>
            <div className='flex flex-col'>
                <p>Playlist</p>
                <h2 className='text-5xl font-bold mb-4 md:text-7xl'>{albumData.name}</h2>
                <h4>{albumData.desc}</h4>
                <p className='mt-1'>
                    <img src={assets.spotify_logo} alt='logo' className='w-5 inline-block'/>
                    <b>Spotify</b>
                    • 1,323,154 likes
                    • <b>50 songs,</b>
                    about 2hr 30 min
                ...
                return (
                    <div onClick={() => playWithId(item.id)} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer' key={index}>
                        <p className='text-white'>
                            ...
                            <img className="inline w-10 mr-5" src={item.image} alt='item-image'/>
                            {item.name}
                        </p>
